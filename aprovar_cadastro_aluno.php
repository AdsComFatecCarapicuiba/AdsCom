<?php
	...
	if ($_SESSION['nivel_especial'] == "1"){
		$consulta = "SELECT * FROM alunos where niveis_acesso_id=3"; // selecionar todos alunos nao cadastrados
		$con = $conn -> query ($consulta) or die ($conn);	
	}else if ($_SESSION['nivel_especial'] == "0"){
		
		$periodo = $_SESSION['periodo'];
		$ciclo = $_SESSION['ciclo'];
		$consulta = "SELECT * FROM alunos where niveis_acesso_id=3 and periodo='$periodo' and ciclo='$ciclo'"; // selecionar todos alunos nao cadastrados da turma do admistrador
		$con = $conn -> query ($consulta) or die ($conn);	
	}	
...
<html lang="pt-br">
  <head>
    ...
    <title>Cadastros de Alunos</title>
  ...
  <div class="container">
    ...
    <div class="container theme-showcase" role="main">      
      <div class="page-header">
        <h1>Aprovar ou Reprovar Cadastro</h1>
		</div>
      <div class="row">
        <div class="col-md-12">
          <table class="table">
            ...
            <tbody>
			    ...
			    while($linhas = $con-> fetch_array()){ 
					echo "<tr>";
						...
						echo "<td>".$linhas['RA']."</td>";
						echo "<td>".$linhas['celular']."</td>";
						echo "<td>".$linhas['ciclo']."</td>";
						echo "<td>".$linhas['periodo']."</td>";
						?>
						<td> 
						<a href='btn_aprovar_aluno.php?id=<?php echo $linhas['id']; ?>'><button type='button' class='btn btn-sm btn-primary'>APROVAR</button></a>
						<a href='btn_reprovar_aluno.php?id=<?php echo $linhas['id']; ?>'><button type='button' class='btn btn-sm btn-danger'>REPROVAR</button></a>
						<?php
					echo "</tr>";
				}
			?>
